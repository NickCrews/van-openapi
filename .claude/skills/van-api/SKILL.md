...
- Six scalar person fields — `nickname`, `website`, `jobTitle` and the three `contactMethodPreference*` fields — are also gated behind `$expand=preferences`. Write one and read the person back plainly and the write looks discarded. ([`update-writes-every-scalar-field`](reference/behaviors/update-writes-every-scalar-field.md))
