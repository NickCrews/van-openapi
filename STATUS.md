...
| People — CRUD | `GET/POST /people`, `POST /people/{vanId}` (update), `POST /people/create`, `POST /people/findOrCreate`, `GET /people/quickSearch` | docs, van-cli | researched (probed findOrCreate, quickSearch) | 2026-08-03 |
...
- **Stateful testing**: the spec now defines OpenAPI `links` from `POST /people/find` (302 and redirect-followed 200) to `GET /people/{vanId}` via `$response.body#/vanId`; schemathesis's stateful phase picks them up automatically in `make fuzz` (~226 scenarios). The 200-response link can't be exercised by schemathesis's raw (non-redirect-following) client — only the 302 link is covered.
